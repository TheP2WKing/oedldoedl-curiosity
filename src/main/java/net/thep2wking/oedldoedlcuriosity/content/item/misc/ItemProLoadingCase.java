...
public class ItemProLoadingCase extends ModItemBase {
	public ItemProLoadingCase(String modid, String name, CreativeTabs tab, EnumRarity rarity, boolean hasEffect,
		...
		if (!worldIn.isRemote) {
			worldIn.spawnEntity(
					new EntityItem(worldIn, playerIn.getPosition().getX(), playerIn.getPosition().getY() + 0.5,
							playerIn.getPosition().getZ(), new ItemStack(ModItems.MINEPODS_PRO)));
